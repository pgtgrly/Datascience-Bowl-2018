...
from data_augment import network3_augment
...
optimizer_checkpoints_directory_network_3="optimizer_checkpoints_network_3"
graphs_network_3_directory="graphs_network_3"
test_batch_size=50
threshold=128
...
class ImageDataset(Dataset): #Defining the class to load datasets
    ...
    def __getitem__(self,idx):
        ...
        input_net_2= input_net_2.reshape((128,128,3))
        ...
        sample = {'image': image, 'masks': masks, 'input_net_1':input_net_1, 'input_net_2':input_net_2}  

        if self.transform:
            sample=network3_augment(sample,vertical_prob=0.5,horizontal_prob=0.5)
        ...
        sample['input_net_2']= sample['input_net_2'].transpose((2, 0, 1))#The convolution function in pytorch expects data in format (N,C,H,W) N is batch size , C are channels H is height and W is width. here we convert image from (H,W,C) to (C,H,W)
        ...
        sample['input_net_2'].astype(float)
        sample['input_net_2']=sample['input_net_2']/255
...
if os.path.exists(checkpoints_directory_network_3) and len(os.listdir(checkpoints_directory_network_3)):
...
elif not os.path.exists(checkpoints_directory_network_3):
    os.makedirs(checkpoints_directory_network_3)

if not os.path.exists(graphs_network_3_directory):
    os.makedirs(graphs_network_3_directory)
...
if  os.path.exists(optimizer_checkpoints_directory_network_3) and len(os.listdir(optimizer_checkpoints_directory_network_3)):
    checkpoints = os.listdir(optimizer_checkpoints_directory_network_3)
    checkpoints.sort(key=lambda x:int((x.split('_')[2]).split('.')[0]))
    optimizer.load_state_dict(torch.load(optimizer_checkpoints_directory_network_3+'/'+checkpoints[-1])) # is this check or should it be checkpoints? changed it to checkpoints.    print("Resuming optimizer")
    print("Resuming Optimizer from iteration " + str(iteri))
elif not os.path.exists(optimizer_checkpoints_directory_network_3):
    os.makedirs(optimizer_checkpoints_directory_network_3)
...
for epoch in range(num_epochs):
    ...
    for i,datapoint in enumerate(train_loader):
        ...
        outputs = model(image, output_network_1, output_network_2)
        ...
        if iteri % 500 ==0:
            ...
            torch.save(model,checkpoints_directory_network_3+'/model_iter_'+str(iteri)+'.pt')
            torch.save(optimizer.state_dict(),optimizer_checkpoints_directory_network_3+'/model_iter_'+str(iteri)+'.pt')
            ...
            writer.export_scalars_to_json(graphs_network_3_directory+"/all_scalars_"+str(iter_new)+".json") #saving loss vs iteration data to be used by visualise.py
    scheduler.step()            
writer.close()