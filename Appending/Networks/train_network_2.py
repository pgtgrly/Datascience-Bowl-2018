...
class ImageDataset(Dataset): #Defining the class to load datasets
    ...
    def __getitem__(self,idx):
        ...
        input_image=cv2.resize(input_image,(128,128), interpolation = cv2.INTER_CUBIC)
        input_image= input_image.reshape((128,128,3)).transpose((2, 0, 1)) #The convolution function in pytorch expects data in format (N,C,H,W) N is batch size , C are channels H is height and W is width. here we convert image from (H,W,C) to (C,H,W)
        ...
        output_image=cv2.resize(output_image,(128,128), interpolation = cv2.INTER_CUBIC)
        output_image= output_image.reshape((128,128,1)).transpose((2, 0, 1))                                                                             

        sample = {'image': input_image, 'masks': output_image, 'input_net_1':input_image_net_1}  
...
if not os.path.exists(checkpoints_directory_network_2):
...
for epoch in range(num_epochs):
    ...
    for i,datapoint in enumerate(train_loader):
        ...
        outputs = model(input_image, output_network_1)
        ...
        if iteri % 500 ==0:
            ...
            torch.save(model,'checkpoints_network_2/model_iter_'+str(iteri)+'.pt')
